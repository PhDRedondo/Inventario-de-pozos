export const es = {
  ...
  appDocs: {
    ...
    architectureBody:
      "Arquitectura institucional en capas separadas (ANH-GTIC-MA-02). El piloto Next.js es la implementación interina, con la misma funcionalidad portada 1:1 y verificada por pruebas de paridad:",
    architectureItems:
      "Presentación: SPA en Angular (componentes standalone), mapa Leaflet y radar/Sankey en SVG; consume la Web API por HTTP con token Bearer.|Web API: ASP.NET Core (.NET 8) Minimal APIs — cuadernos, cargas, validaciones, estadísticas, analítica y mapa; Swagger en desarrollo.|Lógica de dominio (C#): validación por reglas, generación del UWI, ETL geográfico/DANE e ingesta de Excel (ClosedXML), con pruebas de paridad contra el piloto.|Persistencia: SQL Server (esquema [vip]) mediante EF Core 8 con migraciones versionadas (InitialCreate).|Geografía: polígonos departamentales y municipales (GeoJSON DANE) y capas base OSM.|Piloto interino: Next.js App Router + React + SQLite (better-sqlite3), desplegado en Vercel para demostración.",
    stackTitle: "Stack tecnológico",
    stackBody: "Componentes principales del stack institucional (objetivo productivo) y del piloto:",
    stackItems:
      "Frontend institucional: Angular 18 (SPA) · el piloto usa Next.js 16 / React 19 / TypeScript.|Backend: .NET 8 / C# (ASP.NET Core Minimal APIs) · EF Core 8 · SQL Server 2019/2022.|Autenticación: Microsoft Entra ID (OpenID Connect / JWT) con MSAL en el SPA y MFA por Acceso Condicional.|Excel: ClosedXML (generación de plantillas e ingesta) · Leaflet, radar y Sankey en SVG.|Pruebas: xUnit (backend) y Karma/Jasmine (Angular), con paridad contra el piloto.|i18n ES/EN · tema claro/oscuro (piloto).",
    securityTitle: "Seguridad y control de acceso",
    securityBody: "Controles del stack institucional (visibles solo para el perfil administrador):",
    securityItems:
      "Autenticación: JWT Bearer contra Entra ID con validación fail-closed (la API no arranca sin Authority/Audience) y MFA exigida por Acceso Condicional.|Autorización por rol: políticas ASP.NET Core (OperatorOrAdmin, ReadInventory, AnhOrAdmin); el rol ANH solo ve inventario validado.|Alcance de datos por operadora: los cuadernos en borrador no salen del espacio de la operadora.|Cabeceras de seguridad (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, HSTS) y CORS restringido.|Notificación por SMTP institucional al aplicar el envío del inventario.|Secretos por entorno/Key Vault, sin credenciales en el código (en el piloto: sesión por cookie firmada con HMAC).",
  ...
  analytics: {
    ...
    choroplethTitle: "Mapa territorial por indicador",
    choroplethHint: "Departamentos coloreados por su valor frente al promedio nacional (100%). Elija el indicador; pase el cursor por un departamento para ver el detalle.",
    choroplethMetric: "Indicador",
    vsNational: "vs promedio nacional",
    noSample: "Sin pozos en la muestra",
    ...
    rawSubtitle: "Valor nacional y de la entidad seleccionada, con su variación relativa.",
    rawMetricCol: "Indicador",
    rawSelectHint: "Seleccione una entidad para comparar y ver la variación.",
    rawEmptyTheme: "Sin datos para este tema en el inventario del piloto.",
