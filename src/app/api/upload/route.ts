...
import { resolveDaneCodes, saveUploadBatch } from "@/lib/db";
...
export async function POST(request: NextRequest) {
  const user = requireRole(requireSession(request), ["operadora", "admin", "anh"]);
  ...
  const batchStatus = user.role === "operadora" ? "draft" : "processed";

  const { upload, results } = saveUploadBatch(file.name, operadora, records, {
    status: batchStatus,
    forceOperadora: user.role === "operadora" ? operadora : null,
  });
  const summary = summarizeValidation(results);

  return NextResponse.json({ upload, summary, results });
}
